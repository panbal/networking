...
while counter<2:
    user_input = input("Enter a Device-Group (or 'quit' to stop): ")
...
for x in target_templates:
    url = f"{base_url}?location=device-group&device-group="+x+"&name="+prvname
...
for y in target_templates:
    url = f"{base_url}?location=device-group&device-group="+y+"&name="+pubname
...
for y in target_templates:
    url = f"{base_url}?location=device-group&device-group="+y+"&name="+coid+"_INTERFACE_NETWORKS"
